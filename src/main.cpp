#include "Arduino.h"
#include <SoftwareSerial.h>

const uint8_t crcTable[] = {
        ,
};;

SoftwareSerial mcSerial(10, 11);

uint16_t productId = 0;
uint16_t firmwareVersion = 0;

uint8_t calculateCrc(const uint8_t *buffer, uint8_t length) {
    uint8_t crc = 0;
    for (uint8_t i = 0; i < length; i++)
        crc = crcTable[crc ^ buffer[i]];
    return crc;
}

void setup() {
    Serial.begin(9600);
    mcSerial.begin(115200);

    uint8_t buffer[8];

    // reinitialize
    memset(buffer, 0, 8);
    buffer[0] = 0x96;
    buffer[1] = calculateCrc(buffer, 1);
    mcSerial.write(buffer, 2);

    // clear reset flags
    memset(buffer, 0, 8);
    uint16_t flags = 1 << 9;
    buffer[0] = 0xA9;
    buffer[1] = (uint8_t) (flags & 0x7F);
    buffer[2] = (uint8_t) (flags >> 7 & 0x7F);
    buffer[3] = calculateCrc(buffer, 4);
    mcSerial.write(buffer, 4);

    // get firmware version
    memset(buffer, 0, 8);
    buffer[0] = 0x87;
    buffer[1] = calculateCrc(buffer, 1);
    mcSerial.write(buffer, 2);

    // read the response
    memset(buffer, 0, 8);
    mcSerial.flush();
    mcSerial.readBytes(buffer, 4);

    productId = buffer[0] | (buffer[1] << 8);
    firmwareVersion = buffer[2] | (buffer[3] << 8);

    Serial.print("Product ID: 0x");
    Serial.println(productId, HEX);
    Serial.print("Firmware Version: 0x");
    Serial.println(firmwareVersion, HEX);
}

void loop() {}
